...
interface ExamTimetableEntry {
  ...
  exams: { name: string; form_level: string; term: string; academic_year: string; } | null;
}
...
export default function StudentExamTimetableTab({ studentId, formLevel, showAll }: Props = {}) {
  ...
  const offline = useOfflineSection<{ entries: ExamTimetableEntry[]; form: string | null }>({
    section: `student.examTimetable.${showAll ? "all" : formLevel ?? "auto"}`,
    userId: user?.id ?? studentId ?? "anon",
    deps: [user?.id, studentId, formLevel, showAll],
    fetcher: async () => {
      let resolvedForm = formLevel || null;
      if (!showAll && !formLevel) {
        if (!user?.id && !studentId) {
          setEntries([]);
          setStudentForm(null);
          return { entries: [], form: null };
        }
        let q = supabase.from("students").select("form");
        if (studentId) q = q.eq("id", studentId);
        else q = q.eq("user_id", user!.id);
        const { data: student } = await q.maybeSingle();
        resolvedForm = student?.form || null;
        if (!resolvedForm) {
          setEntries([]);
          setStudentForm(null);
          return { entries: [], form: null };
      ...
      let query = supabase
        ...
        .select(`id, exam_date, start_time, end_time, venue, notes,
          subjects(name, code),
        ...
        .order("exam_date")
        .order("start_time");
      if (!showAll && resolvedForm) query = query.eq("exams.form_level", resolvedForm);

      const { data } = await query;
      const rows = (data as ExamTimetableEntry[]) || [];
      setEntries(rows);
      setStudentForm(showAll ? "all" : resolvedForm);
      return { entries: rows, form: showAll ? "all" : resolvedForm };
    },
    restore: (cached) => {
      setEntries(cached?.entries || []);
      setStudentForm(cached?.form ?? null);
    },
  });
